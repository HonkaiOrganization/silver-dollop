...
def _slice_window_frames(video_path: str, start_frame: int, end_frame: int, max_frames: int = 8) -> list[str]:
    """Extract frames from the specified window range, uniformly sampling up to max_frames, returning base64-encoded list."""
    ...
    if start_frame >= end_frame:
        cap.release()
        raise ValueError(f"Invalid window frame range: {start_frame}-{end_frame}")
...
def _build_image_contents(encoded_frames: list[str], prob_abnormal: float, figure_number: int) -> list[dict]:
    """Build the content list for multimodal message: multiple images + text description."""
    content = []
...
def _extract_clip_b64(
    ...
    """Extract a clip from the specified frame range, encoded as base64 MP4."""
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        return None
...
def analyze_windows(
    ...
    """
    Based on the prob_abnormal ranking in window_details from the inference JSON,
    extract top-k most problematic windows and analyze each with VLM.
    ...
    if api_key is None:
        ...
        logger.info("Using environment variable DASHSCOPE_API_KEY: %s", api_key)
        if api_key is None:
            return {
    ...
    for idx, w in enumerate(top_windows, 1):
        ...
        try:
            ...
            md_sections.append(f"> 截取帧失败: {e}")
            md_sections.append("")
            continue

        if not encoded:
            md_sections.append("> 未提取到有效帧")
            md_sections.append("")
            continue
...
if __name__ == "__main__":
    ...
    @click.command()
    @click.option('--video', required=True, type=click.Path(exists=True), help='Jump rope video file path')
    @click.option('--json', 'json_path', required=True, type=click.Path(exists=True), help='JumpRopeInference output JSON file path')
    @click.option('--top_k', default=3, type=int, help='Number of windows with highest prob_abnormal to select (default: 3)')
    @click.option('--max_frames', default=8, type=int, help='Maximum frames to extract per segment (default: 8)')
    @click.option('--api-key', default=None, help='DashScope API Key, overrides DASHSCOPE_API_KEY env var')
    @click.option('--output', default='vlm_report.md', type=click.Path(), help='Analysis result output file path')
